                            ...
                            <form onSubmit={handleSubmit}>
                                <div className="glass-panel border-2 border-accent-red/50 shadow-glowRed bg-bg-primary/95 p-6 md:p-8">
                                    ...
                                    <div className="space-y-5">
                                        ...
                                        <div>
                                            <label className="text-xs font-mono text-text-secondary tracking-wider block mb-2">SCAN INTENSITY</label>
                                            <div className="grid grid-cols-3 gap-2">
                                                {scanProfiles.map((p) => (
                                                    <button
                                                        ...
                                                        className={`p-2 md:p-3 rounded-xl border transition-all duration-300 text-center cursor-pointer ${profile === p.id
                                                                ? 'bg-accent-red/20 border-accent-red shadow-glowRed'
                                                                : 'bg-bg-secondary/50 border-accent-red/30 hover:border-accent-red/60'
                                                            }`}
                                                    >
                                                        <p.icon className={`w-4 h-4 mx-auto mb-1 ${profile === p.id ? 'text-accent-red' : 'text-text-dim'}`} />
                                                        <span className={`text-xs font-bold font-mono ${profile === p.id ? 'text-accent-red' : 'text-text-primary'}`}>{p.label}</span>
                                                        <span className="text-[9px] font-mono text-text-dim block">{p.time}</span>
                                                    </button>
                                                ))}
                                            </div>
                                        </div>
