...
import { initiateScan, getScanStatus, getScanResults } from '../../services/scanService';
...
const ScanInit = ({ onComplete }) => {
    const { setScanData, setScanProgress, targetDomain, setTaskId } = useDashboard();
    ...
    useEffect(() => {
        const startScan = async () => {
            try {
                ...
                const response = await initiateScan(target, 'full-scan');
                setRealTaskId(response.taskId);
        ...
        startScan();
    }, [targetDomain, setScanProgress, setTaskId]);

    // Step 2: Poll for status updates
    useEffect(() => {
        if (!realTaskId || !isScanning) return;
        ...
        const pollStatus = async () => {
            try {
                const statusData = await getScanStatus(realTaskId);
                ...
                if (statusData.status === 'completed') {
                    ...
                    const results = await getScanResults(realTaskId);
                    setRealScanData(results); // Store it here, handleViewReport will use it
    ...
    }, [realTaskId, isScanning, activeStep, setScanProgress]);
                ...
                >
                    ...
                    <h1 className="text-5xl md:text-6xl font-black mb-3">
                        <span className="bg-gradient-to-r from-text-primary via-accent-red to-text-primary bg-clip-text text-transparent">
                            {scanError ? "INITIALIZATION FAILED" : "INITIALIZING"}
                        </span>
                        <br />
                    ...
                    <div className="flex items-center justify-center gap-3">
                        ...
                        {!scanError && <div className="w-1.5 h-1.5 rounded-full bg-accent-cyan animate-pulse" />}
                    </div>
                    
                    {scanError && (
                        <div className="mt-4 p-4 border border-accent-red/30 bg-accent-red/10 text-accent-red rounded-lg font-mono text-sm">
                            {scanError}
                        </div>
                    )}
                </motion.div>
